...
enum
{
	...
	template<> inline uint16 read(const uint16 d) {
		return rev16(d);
	}
	
	template<> inline int16 read(const int16 d) {
		return rev16(d);
	}

	template<> inline uint32 read(const uint32 d) {
		return rev32(d);
	}
...
#pragma pack(1)	// We need this or the structure members aren't alligned 
	...
	struct NameRecord
	{
		uint16	platform_id,
			...
			length,
			offset;
		enum	{Unicode, Macintosh, Reserved, Microsoft};
		enum	
		{
