...
public class CppCodeGenerator extends CodeGenerator {
	...
	public void gen(ActionElement action) {
		...
		if ( action.isSemPred ) {
			...
			if ( tInfo.assignToRoot ) {
				...
				println("currentAST.child = "+tInfo.refRuleRoot+"!=nullAST &&"+tInfo.refRuleRoot+"->getFirstChild()!=nullAST ?");
				tabs++;
	...
	public void genBody(LexerGrammar g) throws IOException {
		...
		println("#include \"antlr/ScannerException.hpp\"");
		...
		if (grammar.debuggingOutput)
			println(": " + sup + "(new DebuggingInputBuffer(new CharBuffer(in)))");
		else
			println(": " + sup + "(new CharBuffer(in))");
		tabs--;
		println("{");
		tabs++;
		...
		println(grammar.getClassName() + "::" + grammar.getClassName() + "(InputBuffer& ib)");
		tabs++;
		// if debugging, wrap the input buffer in a debugger
		if (grammar.debuggingOutput)
			println(": " + sup + "(new DebuggingInputBuffer(ib))");
		else
			println(": " + sup + "(ib)");
		tabs--;
		println("{");
		...
		println(grammar.getClassName() + "::" + grammar.getClassName() + "(const LexerSharedInputState& state)");
		tabs++;
		println(": " + sup + "(state)");
		tabs--;
		println("{");
	...
	public void genBody(ParserGrammar g) throws IOException {
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(TokenStream& lexer, int k)");
		println(": " + sup + "(lexer,k)");
		println("{");
		tabs++;
		println("setTokenNames(_tokenNames);");
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(const ParserSharedInputState& state)");
	...
	protected void genHeader() 
	{
		...
		println(" * ANTLR Version " + Tool.version + "; 1996-1999");
		println(" */");
		println("");
	}
...
/*
		println(" implements " + grammar.tokenManager.getName() + "TokenTypes, TokenStream");
*/
		...
		// Generate the constructor from InputBuffer
		println(grammar.getClassName() + "(InputBuffer& ib);");

		println(grammar.getClassName() + "(const LexerSharedInputState& state);");
...
//		println("#include \"antlr/ParserSharedInputState.hpp\"");
		...
		println("protected:");
		tabs=1;
		println(grammar.getClassName()+"(TokenStream& lexer, int k);");
		tabs=0;
		println("public:");
		tabs=1;
		println(grammar.getClassName()+"(TokenStream& lexer);");

		println(grammar.getClassName()+"(const ParserSharedInputState& state);");
	...
	public void genNextToken() {
		...
		println("");
		println("RefToken "+grammar.getClassName()+"::nextToken()");
		println("{");
		tabs++;
...
//		}
		...
		if ( grammar instanceof LexerGrammar ) {
			println("if ( _createToken && _token==nullToken && _ttype!=Token::SKIP ) {");
...
//		}
	...
	protected void genSemPred(String pred, int line) {
		...
		println("if (!(" + pred + "))");
		tabs++;
		println("throw SemanticException(\"" + escapedPred + "\");");
		tabs--;
	}
...
//		println("");
	...
	protected void genTokenTypes(TokenManager tm) throws IOException {
		...
		println("enum " + tm.getName() + TokenTypesFileSuffix+" {");
		tabs++;
	...
	private void setupGrammarParameters(Grammar g) {
		if (g instanceof ParserGrammar) {
			...
			lt1Value = "_t";
			exceptionThrown = "ParserException";
			throwNoViable = "throw NoViableAltException(_t);";
		}
		else {
