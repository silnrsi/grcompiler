...
Description:
    Implements the methods for TtfUtil namespace. This file should remain portable to any C++ 
...
{
	lOffset = 0;
	lSize   = sizeof(Sfnt::OffsetSubTable) - sizeof(Sfnt::OffsetSubTable::table_directory);
...
int PostLookup(const void * pPost, size_t lPostSize, const void * pMaxp, const char * pPostName)
{
	...
	{ 
		...
		auto cnGlyphs = GlyphCount(pMaxp);
						   ...
						   size_t lLocaSize, const void * pHead)
{
	...
	auto lGlyfOffset = LocaLookup(nGlyphId, pLoca, lLocaSize, pHead);
...
/*----------------------------------------------------------------------------------------------
	...
	Return true if successful, false otherwise. On false, all end points are INT_MIN
		False may indicate a white space glyph (or component)
----------------------------------------------------------------------------------------------*/
bool GlyfContourEndPoints(gr::gid16 nGlyphId, const void * pGlyf, 
...
---------------------------------------------------------------------------------------------*/
bool SimplifyFlags(char * prgbFlags, size_t cnPoints)
{
	for (size_t i = 0; i < cnPoints; i++)
