...
#endif
{
	...
	// First try it with just the tmpnam generated by the system; then make sure that has a full valid path.
	tmpnam(pszPreProcErr);
	int i = 0;
	for (int i = strlen(pszPreProcErr) - 1; i >= 0; i--)
	{
		if (pszPreProcErr[i] == '\\' || pszPreProcErr[i] == '/')
			break;
	}
	if (i < 0)
	{
		// No valid path - put it in the output path.
		strcpy(pszPreProcErr, staOutputPath.c_str());
		pszPreProcErr[cchOutputPath] = '/';
		tmpnam(pszPreProcErr + cchOutputPath + 1);
	}

	if (pszPreProcErr[0] == '\\' || pszPreProcErr[0] == '/')
